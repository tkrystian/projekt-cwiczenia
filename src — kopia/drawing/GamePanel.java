...
public class GamePanel extends JPanel implements KeyListener, ActionListener, MouseWheelListener {
    ...
    private void initializeGame() {
        try {
            ...
            brickWalls = new BrickWalls(grid).getBrickWalls();
            stones = new Stones(grid).getStones();
            enemies = new Enemies(grid, gameStats);
            player = new Player(gameStats, grid);
        } catch (IOException e) {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (gameStats.isGameOver()) {
            ...
            if (selectedOptionInMenu == -1) {
                gameRenderer.drawGame(g, this, player, bomb, enemies, brickWalls, stones);
                gameRenderer.drawScoreboard(g, player);
            } else {
